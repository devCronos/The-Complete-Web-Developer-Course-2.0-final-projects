    ...
    if($_POST["loginActive"] == "0"){
        ...
        if(mysqli_num_rows($result) > 0) $error = "This email address is already taken.";
            ...
            if (mysqli_query($link,$query)){
                ...
                echo 1;
            }else $error = "Coult not connect to the server. Please try again later.";
        }
    }else{
        $query = "SELECT * FROM `twitter`.`users` WHERE `email` = '".mysqli_real_escape_string($link,$_POST['email'])."' LIMIT 1 ";
        $result = mysqli_query($link, $query);
        ...
        if($row['password'] == md5(md5($row['id']).$_POST['password'])) {
            echo 1;
            $_SESSION['id'] = $row['id'];
        }else $error = "Coult not find the combination. Please try again.";
...
if ($_GET['action'] == 'toggleFollow') {
    $query = "SELECT * FROM `twitter`.`isfollowing` WHERE `follower` = ".mysqli_real_escape_string($link,$_SESSION['id'])."  AND `following` = ".mysqli_real_escape_string($link,$_POST['userId'])." LIMIT 1 ";
    $result = mysqli_query($link, $query);
    if(mysqli_num_rows($result) > 0) {
        ...
        mysqli_query($link,"INSERT INTO `isfollowing` (follower, following) VALUES(".mysqli_real_escape_string($link,$_SESSION['id']).",".mysqli_real_escape_string($link,$_POST['userId']).")");
        echo "2";
    }
}
...
if ($_GET['action'] == 'postTweet') {
    if (!$_POST["tweetContent"]) {
        ...
        echo "Your tweet exceeded 140 characters";
    }else {
